...
bool CollisionWorld::move(Player& player, PortalMotion& motion, const MovementInput& input, Direction gravity) const {
    ...
    const Vec2 gravityAxis = directionVector(gravity);
    const bool vertical = gravityAxis.x == 0;
    const Vec2 tangent = vertical ? Vec2{1, 0} : Vec2{0, 1};
    const bool negative = vertical ? input.left : input.jump;
    const bool positive = vertical ? input.right : input.down;
    const bool jump = gravity == Direction::Down ? input.jump :
                      gravity == Direction::Up ? input.down :
                      gravity == Direction::Right ? input.left : input.right;
    const bool grounded = blocked(gravityAxis * 0.02);
    const double acceleration = negative ? -1.0 : (positive ? 1.0 : 0.0);
    double lateral = dot(player.velocity, tangent);
    double falling = dot(player.velocity, gravityAxis);
    if (std::abs(lateral) <= 6.0)
        lateral = std::clamp(lateral + acceleration, -6.0, 6.0);
    else if (lateral * acceleration < 0) lateral += acceleration;
    else lateral *= 0.99;
    if (grounded && !negative && !positive) lateral *= 0.5;
    if (std::abs(lateral) < 0.01) lateral = 0;
    // Legacy -7 was immediately followed by +1 gravity before its first movement.
    if (jump && grounded) falling = -6.0;
    else if (grounded && falling >= 0) falling = 0;
    else falling = std::min(falling + 1.0, 20.0);
    player.velocity = tangent * lateral + gravityAxis * falling;
