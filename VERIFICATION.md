...
`Por2D/build.ps1 -Test` 构建通过，8 组共 798,409 项检查通过。新增检查覆盖四种重力与四种身体朝向、四个屏幕方向键、支撑面跳跃、空中禁跳、顺重力键无效果、重力循环和重开复位，并在 17 张地图上进行 10,200 帧切换重力与射门的碰撞检查。

隐藏窗口检查通过，覆盖菜单 Enter 开始、E 过关事件、G 单次切换、长按不连发、同帧多次按 G、鼠标选关、全屏恢复与绘制。窗口检查是自动输入验证，不代表全部关卡都已人工通关。

以下保留复制前的验证记录，其中旧路径和旧测试数量属于原版。
