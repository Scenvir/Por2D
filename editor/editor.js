...
function controls() { $('name').value = map.name; $('commentary').value = map.commentary; if(map[tool]) $('direction').value = map[tool].direction; }
...
function draw() {
  ...
  if(stroke && stroke.rectangle && stroke.last) {
...
function cancel(){if(stroke){map=M.parse(stroke.before);stroke=null;draw();}}
canvas.addEventListener('contextmenu',e=>e.preventDefault());
canvas.addEventListener('pointerdown',e=>{if(stroke||e.button!==0&&e.button!==2||!cellAt(e))return;e.preventDefault();stroke={before:snapshot(),erase:e.button===2,last:null,start:cellAt(e),tool,rectangle:$('shape').value==='rectangle'&&['0','1','2'].includes(tool)};canvas.setPointerCapture(e.pointerId);move(e);});
...
function travel(from,to){finish();if(!from.length)return;to.push(snapshot());map=M.parse(from.pop());markChanged();controls();draw();}
