...
struct ReplayScript {
    ...
    static ReplayScript parse(std::istream& stream) {
        ...
        while (std::getline(stream,line)) {
            ...
            if (op=="f") {
                int flags=0,count=0;
                if(!(row>>action.frames>>flags>>count)||action.frames<1||action.frames>1000000||flags<0||flags>511||count<0||count>256)fail();
                ...
                action.input.movement.down=flags&64;
                action.input.gravityTurns=(flags>>7)&3;
...
struct Recorder {
    ...
    static std::string encode(const InputFrame& input) {
        ...
        const int flags=input.movement.left+2*input.movement.right+4*input.movement.jump+8*input.restart+16*input.useExit+32*input.skip+64*input.movement.down+128*(input.gravityTurns&3);
