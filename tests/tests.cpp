...
void mapsAndTransforms() {
    const std::array<const char*,15> names{{u8"概念",u8"高山",u8"旋转",u8"远跳",u8"大脑",
        u8"穿梭",u8"支点",u8"裂缝",u8"铁砧",u8"倒立",u8"远见",u8"生死逆转",u8"阀门",u8"深渊",u8"生死逆转"}};
...
void gravityControls() {
    using namespace por2;
    TileMap room;
    room.fill(Tile::Solid,0,0,49,3);
    room.fill(Tile::Solid,0,26,49,29);
    room.fill(Tile::Solid,0,4,3,25);
    room.fill(Tile::Solid,46,4,49,25);
    const CollisionWorld world(room,{});
    const std::array<MovementInput,4> keys{{{false,false,true,false}, {false,true,false,false},
                                           {false,false,false,true}, {true,false,false,false}}};
    for(int g=0;g<4;++g) for(int orientation=0;orientation<4;++orientation) {
        const auto gravity=static_cast<Direction>(g);
        const auto axis=directionVector(gravity);
        Player player{{{400,300},static_cast<Direction>(orientation)},{},{}};
        PortalMotion motion;
        for(int i=0;i<100;++i) expect(world.move(player,motion,{},gravity),"four-direction fall remains clear");
        const auto bounds=player.body.bounds();
        expect(g==0?near(bounds.top,80):g==1?near(bounds.right,920):g==2?near(bounds.bottom,520):near(bounds.left,80),
               "lands against gravity-facing surface");
        for(int key=0;key<4;++key) {
            auto moved=player; auto state=motion;
            expect(world.move(moved,state,keys[key],gravity),"screen-direction control remains clear");
            if(key==g) expect(near(moved.body.position,player.body.position),"gravity-direction key has no effect");
            else if(key==(g+2)%4) expect(near(dot(moved.velocity,axis),-6),"opposite key jumps away from support");
            else expect(dot(moved.velocity,directionVector(static_cast<Direction>(key)))>0,"lateral key follows screen direction");
            expect(moved.body.direction==player.body.direction,"gravity does not rotate body");
        }
        auto airborne=player; auto state=motion;
        airborne.body.position=airborne.body.position-axis*50;
        expect(world.move(airborne,state,keys[(g+2)%4],gravity),"airborne motion valid");
        expect(dot(airborne.velocity,axis)>0,"no airborne jump without support");
    }
    Game game;
    InputFrame change; change.gravityTurns=1;
    for(auto direction:{Direction::Left,Direction::Up,Direction::Right,Direction::Down}) {
        game.tick(change); expect(game.gravity()==direction,"G cycles all four directions");
    }
    game.tick(change); game.restart();
    expect(game.gravity()==Direction::Down,"restart resets gravity");
    std::mt19937 random(741);
    for(int id=0;id<=16;++id) {
        Game sample(id);
        for(int frame=0;frame<600;++frame) {
            InputFrame input;
            input.movement={random()%4==0,random()%4==0,random()%4==0,random()%4==0};
            input.gravityTurns=frame%13==0?1:0;
            if(frame%7==0) input.shots.push_back({static_cast<int>(random()%2),{random()%1000,random()%600}});
            sample.tick(input);
            const CollisionWorld current(sample.level().map,sample.portals());
            expect(current.canOccupy(sample.player().body,sample.motion().activePortal),"gravity switching and portal shots never embed body");
            expect(finite(sample.player().velocity),"gravity switching preserves finite velocity");
        }
    }
}
